...
```{r}
rf.grid <- expand.grid("mtry" = c(20, 40, 80), "splitrule" ="extratrees", "min.node.size" = 1)
...
rf.model
beepr::beep(sound = 2)
rf.preds <- predict(rf.model, test) %>% exp()
rf.preds.frame <- data.frame(id= test$id, loss = rf.preds)
write_csv(rf.preds.frame, "rf-preds.csv")
```

```{r}
set.seed(062520)
grid <- expand.grid("n.trees" = 434, "interaction.depth" = 7, "shrinkage" = c(.01, .05,.075), "n.minobsinnode" = 20)
...
write_csv(gbm.preds.frame, "gbm-preds.csv")
```
